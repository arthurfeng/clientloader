...
def _parse_sdpplin_line(item):
	...
	type, sep, value = value.partition(';')
        ...
        if type == 'buffer':
            strg = value[1:-1]
            try:
                value = base64.b64decode(value[strg])
            except:
                lens = len(strg)
                lenx = lens - (lens % 4 if lens % 4 else 4)
                value = base64.b64decode(strg[:lenx])
        if type == 'string':
            value = value[1:-1]
    return name, value
...
class SdpplinMediaDesc(SDPMediaDesc):
    ...
    def __init__(self, media_desc):
        ...
        #### fengjian ####
        for item in media_desc.m:
            value = item.split(" ")
            if value[0].lower() == "video":
                self.attributes["port"] = value[1]
            elif value[0].lower() == "audio":      
                self.attributes["port"] = value[1]
        for item in media_desc.c:
            value = item.split(" ")
            self.attributes["ip"] = value[-1]
...
if __name__ == "__main__":
    ...
    f.close()
    parser = Sdpplin(sdp)
    print parser.streams[-1].attributes
    print parser.items()
