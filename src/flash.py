...
def _connect(url, params=[], timeout=10, duration=60, publishStream=False, playStream=True, publishFile=False, enableAudio=True, enableVideo=True):
    ...
    playedStream = os.path.split(parsed_url.path)[1]
    connectURL = "%s://%s%s" % (parsed_url.scheme, parsed_url.netloc, os.path.split(parsed_url.path)[0])
    nc = NetConnection()
    result = yield nc.connect(connectURL, timeout, *params)
    if not result: raise StopIteration, 'Failed to connect %r'%(connectURL,)
    ...
    if playStream:
        ...
        result = yield ns2.play(playedStream, timeout=timeout)
        if not result: yield nc.close(); raise StopIteration, 'Failed to create play stream %r'%(playedStream,)
...
if __name__ == '__main__':
    ...
    import threading
    url = "rtmp://192.168.36.159/rtmplive/broadcast/live"
    _debug = True
    for i in range(2):
        client = threading.Thread(target=connect, args=(url,))
        client.start()
